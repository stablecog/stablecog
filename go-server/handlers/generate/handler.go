...
func Handler(c *fiber.Ctx) error {
	...
	promptFlores, negativePromptFlores := shared.GetPromptFloresCodes(cleanedPrompt, cleanedNegativePrompt)
...
func HandlerV2(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&req); err != nil {
		...
		sentry.CaptureException(err)
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: "Invalid request body"},
		)
	}
	if req.Seed == -1 {
		rand.Seed(time.Now().Unix())
	...
	if req.Width > shared.MaxWidth {
		log.Printf("Width is too large: %d", req.Width)
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: fmt.Sprintf("Width is too large, max is: %d", shared.MaxWidth)},
		)
	}
	if req.Height > shared.MaxHeight {
		log.Printf("Height is too large: %d", req.Height)
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: fmt.Sprintf("Height is too large, max is: %d", shared.MaxHeight)},
		)
	}
	if req.Width*req.Height*req.NumInferenceSteps >= shared.MaxProPixelSteps {
		log.Printf(
		...
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: "Pick fewer inference steps or smaller dimensions"},
		)
	}
	if shared.ModelIdToModelNameCog[req.ModelId] == "" {
		log.Printf("Invalid model ID: %s", req.ModelId)
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: "Invalid model ID"},
		)
	}
	if shared.SchedulerIdToSchedulerNameCog[req.SchedulerId] == "" {
		log.Printf("Invalid scheduler ID: %s", req.SchedulerId)
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: "Invalid scheduler ID"},
		)
	}
	...
	if supabaseUserId != "" {
		...
		return c.Status(http.StatusBadRequest).JSON(
			SGenerateResponseV2{Error: "You need to create an account to generate images."},
		)
	}
	...
	if plan == "FREE" {
		...
		if shared.Contains(shared.AvailableWidthsFree, req.Width) == false {
			return c.Status(http.StatusBadRequest).JSON(
				SGenerateResponseV2{Error: "That generation width is not available on the free plan :("},
			)
		}
		if shared.Contains(shared.AvailableHeightsFree, req.Height) == false {
			return c.Status(http.StatusBadRequest).JSON(
				SGenerateResponseV2{Error: "That generation height is not available on the free plan :("},
			)
		}
		if shared.Contains(shared.AvailableInferenceStepsFree, req.NumInferenceSteps) == false {
			return c.Status(http.StatusBadRequest).JSON(
				SGenerateResponseV2{Error: "That inference steps setting is not available on the free plan :("},
			)
		}
		if shared.Contains(shared.AvailableSchedulerIdsFree, req.SchedulerId) == false {
			return c.Status(http.StatusBadRequest).JSON(
				SGenerateResponseV2{Error: "That scheduler is not available on the free plan :("},
			)
		}
	}
	...
	if err != nil {
		...
		shared.DeleteOngoingGenerationOrUpscale("goa_active", supabaseUserId)
		return c.Status(http.StatusInternalServerError).JSON(
			SGenerateResponseV2{Error: "Unable to add request to queue"},
		)
	}
	...
	case response := <-generateChannel:
		if response.Status == shared.WebhookFailed {
			...
			shared.DeleteOngoingGenerationOrUpscale("goa_active", supabaseUserId)
			return c.Status(http.StatusInternalServerError).JSON(
				SGenerateResponseV2{Error: "Cog returned an error"},
			)
		}
		// ! Success response
		if len(response.Output) == 0 {
			...
			shared.DeleteOngoingGenerationOrUpscale("goa_active", supabaseUserId)
			return c.Status(http.StatusInternalServerError).JSON(
				SGenerateResponseV2{Error: "Cog server returned empty output"},
			)
		}
	...
	if req.ShouldSubmitToGallery {
		go SubmitToGalleryV2(SSubmitToGalleryPropsV2{
			PromptIdChan:         promptIdChan,
			NegativePromptIdChan: negativePromptIdChan,
			Output:               output,
			ModelId:              req.ModelId,
	...
	loggers.LogGeneration(fmt.Sprintf("Generation successful result returned in: %v%s", green(end-start), green("ms")), logObj)
	shared.DeleteOngoingGenerationOrUpscale("goa_active", supabaseUserId)
	return c.Status(http.StatusOK).JSON(SGenerateResponseV2{
		Data: SGenerateResponseDataV2{
			ImageUrls:  output,
			DurationMs: generationCogDurationMs,
		},
	})
...
type SGenerateResponseV2 struct {
	Data  SGenerateResponseDataV2 `json:"data,omitempty"`
	Error string                  `json:"error,omitempty"`
}
...
type SGenerateResponseDataV2 struct {

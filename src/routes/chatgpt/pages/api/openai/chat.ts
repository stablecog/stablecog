...
export async function extractOpenaiChatInputs(req: NextRequest): Promise<ApiChatInput> {
  ...
  const api: OpenAIAPI.Configuration = {
    apiKey: (userApi.apiKey || env.OPENAI_API_KEY || '').trim(),
    apiHost: (userApi.apiHost || env.OPENAI_API_HOST || 'api.openai.com').trim().replaceAll('https://', ''),
    apiOrganizationId: (userApi.apiOrganizationId || env.OPENAI_API_ORG_ID || '').trim(),
  };
  if (!api.apiKey)

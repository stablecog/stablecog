<script lang="ts">
	...
	const rootPathnames = ['/docs', '/docs/v1'];
	...
	$: allChildrenAreEndpoints =
		sidebarItem?.children !== undefined
			? sidebarItem?.children?.every((child) => child.http_method)
			: false;
...
<MetaTag
	title="{metadata.title} | {rootPathnames.includes($page.url.pathname) ? 'Stablecog' : 'Docs'}"

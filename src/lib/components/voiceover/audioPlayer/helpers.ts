...
export function drawWaveformBars(options: DrawWaveformBarsOptions): void {
	const {
		values,
		progress,
		element,
	...
	const x = d3.scaleLinear().range([0, width]);
	const y = d3.scaleLinear().range([0, height / 2]);
	...
	values.forEach((d, i) => {
		const exactProgressPosition = values.length * progress;

		if (i < exactProgressPosition && i + 1 <= exactProgressPosition) {
			svg
				.append('rect')
				.attr('x', x(i))
				.attr('y', height / 2 - y(d))
				...
				.attr('rx', barRadius)
				.attr('ry', barRadius);
		} else if (i < exactProgressPosition && i + 1 > exactProgressPosition) {
			const completedWidth = barWidth * (exactProgressPosition - i);
			const remainingWidth = barWidth - completedWidth;

			svg
				.append('clipPath')
				.attr('id', 'clipCompleted')
				.append('rect')
				.attr('x', x(i))
				.attr('y', height / 2 - y(d))
			...
			svg
				.append('clipPath')
				.attr('id', 'clipRemaining')
				.append('rect')
				.attr('x', x(i) + completedWidth)
				.attr('y', height / 2 - y(d))
			...
			svg
				...
				.attr('height', y(d) * 2)
				.attr('fill', `url(#gradient1)`)
				.attr('rx', i == 0 && completedWidth == barWidth ? barRadius : 0)
				.attr('ry', i == 0 && completedWidth == barWidth ? barRadius : 0)
				.attr('clip-path', 'url(#clipCompleted)');
			...
			svg
				...
				.attr('height', y(d) * 2)
				.attr('fill', `url(#gradient2)`)
				.attr('rx', i == values.length - 1 && remainingWidth == barWidth ? barRadius : 0)
				.attr('ry', i == values.length - 1 && remainingWidth == barWidth ? barRadius : 0)
				.attr('clip-path', 'url(#clipRemaining)');
		} else {
			svg
				.append('rect')
				.attr('x', x(i))
				...
				.attr('height', y(d) * 2)
				.attr('fill', `url(#gradient2)`)
				.attr('rx', i == values.length - 1 ? barRadius : 0)
				.attr('ry', i == values.length - 1 ? barRadius : 0);
		}
	});
}

<script lang="ts">
	import type { TIconGenerationSettingsSet } from '$components/icons/iconGenerationSettingsSet/types';

	export { classes as class };
	let classes = 'w-6 h-6';
	export let type: TIconGenerationSettingsSet;
</script>

{#if type === 'aspect-ratio' || type === 'dimensions'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M4 5a1 1 0 0 1 1-1h1a1 1 0 0 0 0-2H5a3 3 0 0 0-3 3v1a1 1 0 0 0 2 0V5Zm14-3a1 1 0 1 0 0 2h1a1 1 0 0 1 1 1v1a1 1 0 1 0 2 0V5a3 3 0 0 0-3-3h-1Zm3 15a1 1 0 0 1 1 1v1a3 3 0 0 1-3 3h-1a1 1 0 1 1 0-2h1a1 1 0 0 0 1-1v-1a1 1 0 0 1 1-1ZM4 18a1 1 0 1 0-2 0v1a3 3 0 0 0 3 3h1a1 1 0 1 0 0-2H5a1 1 0 0 1-1-1v-1Zm17-8.5a1 1 0 0 1 1 1v3a1 1 0 1 1-2 0v-3a1 1 0 0 1 1-1Zm-17 1a1 1 0 0 0-2 0v3a1 1 0 1 0 2 0v-3ZM9.5 3a1 1 0 0 1 1-1h3a1 1 0 1 1 0 2h-3a1 1 0 0 1-1-1Zm1 17a1 1 0 1 0 0 2h3a1 1 0 1 0 0-2h-3Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'model'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M9.13.5a4.8 4.8 0 0 0-4.57 3.34 5.27 5.27 0 0 0-2.82 6.51 5.73 5.73 0 0 0 1.68 8.57A5.27 5.27 0 0 0 12 22.3a5.25 5.25 0 0 0 8.58-3.38 5.75 5.75 0 0 0 1.68-8.57 5.27 5.27 0 0 0-2.82-6.5A4.8 4.8 0 0 0 12 1.44C11.2.86 10.2.5 9.12.5ZM6.3 4.74a2.88 2.88 0 0 1 4.03-2.06c.5.23.71.78.71 1.33V19.79c0 .5-.18 1-.6 1.28A3.35 3.35 0 0 1 5.3 18.3l-.02-.62-.56-.25a3.83 3.83 0 0 1-1.46-5.9h2.03c.53 0 .96.42.96.95v.73a1.92 1.92 0 1 0 1.92 0v-.73A2.87 2.87 0 0 0 5.29 9.6H3.51a3.36 3.36 0 0 1 2.16-4.14l.53-.17.1-.55Zm6.66 12.05h1.91a2.88 2.88 0 0 0 2.88-2.87v-3.13a1.92 1.92 0 1 0-1.92 0v3.13c0 .53-.43.96-.96.96h-1.91V4c0-.55.22-1.1.71-1.33a2.86 2.86 0 0 1 4.03 2.06l.1.55.53.17a3.36 3.36 0 0 1 1.96 4.66l-.3.61.48.48a3.82 3.82 0 0 1-1.19 6.22l-.56.25-.01.61a3.35 3.35 0 0 1-5.14 2.78c-.43-.27-.61-.77-.61-1.28v-3Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'upload-image'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M20.5 1.25a.75.75 0 0 0-1.5 0V3.5h-2.25a.75.75 0 0 0 0 1.5H19v2.25a.75.75 0 0 0 1.5 0V5h2.25a.75.75 0 0 0 0-1.5H20.5V1.25Zm2.25 9.326V18A3.75 3.75 0 0 1 19 21.75H5A3.75 3.75 0 0 1 1.25 18V6A3.75 3.75 0 0 1 5 2.25h8.04c-.143.48-.236.982-.272 1.5H5A2.25 2.25 0 0 0 2.75 6v7.973l3.17-3.7a2.75 2.75 0 0 1 4.166-.01l3.06 3.53 1.91-1.91a2.75 2.75 0 0 1 3.889 0l2.305 2.306v-3.1a6.948 6.948 0 0 0 1.5-.513Zm-6.634 2.368-1.985 1.985.936 1.08a.75.75 0 0 1-1.134.982l-4.98-5.746a1.25 1.25 0 0 0-1.894.005L2.75 16.277V18A2.25 2.25 0 0 0 5 20.25h14A2.25 2.25 0 0 0 21.25 18v-1.69l-3.366-3.365a1.25 1.25 0 0 0-1.768 0ZM13.5 9a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'negative-prompt'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M5.367 3.477C7.205 3.235 9.609 3 12 3s4.795.235 6.633.477C19.993 3.656 21 4.84 21 6.279v7.12a2 2 0 0 1-1.717 1.979l-2.654.38c-1.405.2-2.675.99-3.444 2.21A66 66 0 0 0 12 19.919a66 66 0 0 0-1.185-1.95 4.872 4.872 0 0 0-3.444-2.21l-2.654-.38A2 2 0 0 1 3 13.398V6.28c0-1.44 1.008-2.623 2.367-2.802ZM12 1c-2.51 0-5.008.246-6.894.494C2.685 1.812 1 3.905 1 6.279v7.12a4 4 0 0 0 3.434 3.959l2.654.379a2.872 2.872 0 0 1 2.035 1.298c.65 1.032 1.464 2.375 1.983 3.412a1 1 0 0 0 1.788 0c.519-1.037 1.333-2.38 1.983-3.412a2.872 2.872 0 0 1 2.035-1.298l2.654-.38A4 4 0 0 0 23 13.399V6.28c0-2.374-1.685-4.467-4.106-4.785C17.008 1.246 14.51 1 12 1Zm3.207 5.293a1 1 0 0 1 0 1.414L13.414 9.5l1.793 1.793a1 1 0 0 1-1.414 1.414L12 10.914l-1.793 1.793a1 1 0 0 1-1.414-1.414L10.586 9.5 8.793 7.707a1 1 0 0 1 1.414-1.414L12 8.086l1.793-1.793a1 1 0 0 1 1.414 0Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'number-of-images'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M5 2.25A3.75 3.75 0 0 0 1.25 6v12A3.75 3.75 0 0 0 5 21.75h14A3.75 3.75 0 0 0 22.75 18V6A3.75 3.75 0 0 0 19 2.25H5Zm16.25 11.94V6c0-1.24-1-2.25-2.25-2.25H5c-1.24 0-2.25 1-2.25 2.25v7.97l3.17-3.7a2.75 2.75 0 0 1 4.17 0l3.06 3.52 1.9-1.9a2.75 2.75 0 0 1 3.9 0l2.3 2.3Zm-7.12.74 1.99-1.99a1.25 1.25 0 0 1 1.76 0l3.37 3.37V18c0 1.24-1 2.25-2.25 2.25H5c-1.24 0-2.25-1-2.25-2.25v-1.72l4.3-5.03c.5-.58 1.4-.58 1.9 0l4.98 5.74a.75.75 0 0 0 1.14-.98l-.94-1.08ZM13.5 9a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'guidance-scale'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M2 2.75A.75.75 0 0 1 2.75 2h18.5a.75.75 0 0 1 0 1.5h-2.361l2.839 7.3c.18.463.297.96.192 1.445a3.501 3.501 0 0 1-6.84 0c-.106-.485.012-.982.192-1.444L18.11 3.5H12.75v17h5.5a.75.75 0 0 1 0 1.5H5.75a.75.75 0 0 1 0-1.5h5.5v-17H5.889l2.839 7.3c.18.463.298.96.192 1.445a3.501 3.501 0 0 1-6.84 0c-.106-.485.012-.982.192-1.444L5.11 3.5H2.75A.75.75 0 0 1 2 2.75Zm3.5 3.889L7.39 11.5H3.61L5.5 6.639ZM20.39 11.5 18.5 6.639 16.61 11.5h3.78Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'scheduler'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M11 8a5 5 0 1 1 10 0 5 5 0 0 1-10 0Zm5-7a7 7 0 1 0 0 14 7 7 0 0 0 0-14ZM3 18a3 3 0 1 1 6 0 3 3 0 0 1-6 0Zm3-5a5 5 0 1 0 0 10 5 5 0 0 0 0-10Zm13.5 5a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM16 19.5a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0ZM3 4.5a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0ZM4.5 1a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'inference-steps'}
	<svg class={classes} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M13.5 3.5a2 2 0 0 1 2-2h5a2 2 0 0 1 2 2v17a2 2 0 0 1-2 2h-17a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h4v-4a2 2 0 0 1 2-2h4v-4Zm7 .5a.5.5 0 0 0-.5-.5h-4a.5.5 0 0 0-.5.5v3.5a2 2 0 0 1-2 2H10a.5.5 0 0 0-.5.5v3.5a2 2 0 0 1-2 2H4a.5.5 0 0 0-.5.5v4a.5.5 0 0 0 .5.5h16a.5.5 0 0 0 .5-.5V4Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'seed'}
	<svg
		class={classes}
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M18.922 4.244c-.368-.022-.758-.022-1.148-.022-3.117 0-7.57.667-10.353 3.89-2.19 2.393-3.207 6.054-3.144 10.709.007.535.45.957.984.957h.49c5.121 0 8.906-1.445 11.132-4.222 2.736-3.237 2.982-7.762 2.836-10.475-.024-.444-.353-.81-.797-.837ZM2.056 20.134C1.13 4.184 11.85 2 17.774 2c.735 0 1.394.033 1.95.078a2.37 2.37 0 0 1 2.195 2.196C22.324 9.237 21.804 22 5.751 22c-.612 0-1.258-.034-1.92-.082a1.91 1.91 0 0 1-1.775-1.785ZM14.483 6.766C8.914 8.03 7.191 12.693 6.658 15.46c-.1.518.507.692.778.24 2.45-4.108 5.323-6.516 7.38-7.84.477-.307.22-1.217-.333-1.092Z"
			fill="currentColor"
		/>
	</svg>
{:else if type === 'show-on-profile'}
	<svg
		width="24"
		height="24"
		xmlns="http://www.w3.org/2000/svg"
		fill="none"
		viewBox="0 0 24 24"
		stroke-width="1.5"
		stroke="currentColor"
		class={classes}
	>
		<path
			stroke-linecap="round"
			stroke-linejoin="round"
			d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z"
		/>
	</svg>
{:else if type === 'image-strength'}
	<svg
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
		class={classes}
	>
		<path
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M22 2.25h-3.11a2.5 2.5 0 0 0-4.78 0H2a.75.75 0 0 0 0 1.5h12.11a2.5 2.5 0 0 0 4.78 0H22a.75.75 0 0 0 0-1.5ZM2.75 7a.75.75 0 0 0-1.5 0v11A3.75 3.75 0 0 0 5 21.75h14A3.75 3.75 0 0 0 22.75 18V7a.75.75 0 0 0-1.5 0v8.44l-2.6-2.08a2.75 2.75 0 0 0-3.32-.1l-2.27 1.63a.77.77 0 0 0-.06.05l-3.06-3.06a2.75 2.75 0 0 0-3.88 0l-3.31 3.3V7Zm14.96 7.53 3.54 2.83V18c0 1.24-1 2.25-2.25 2.25H5c-1.24 0-2.25-1-2.25-2.25v-.69l4.37-4.37a1.25 1.25 0 0 1 1.76 0l5.09 5.09a.75.75 0 1 0 1.06-1.06l-.96-.96 2.14-1.52c.45-.33 1.07-.3 1.5.04ZM13.5 11a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"
			fill="currentColor"
		/>
	</svg>
{/if}

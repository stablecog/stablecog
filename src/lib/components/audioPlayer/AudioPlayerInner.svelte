<script lang="ts">
	...
	$: currentTimestamp = convertSecondsToTimestamp(currentTime);
	$: totalTimestamp = convertSecondsToTimestamp(duration);
...
<div class="w-full bg-c-bg-secondary pl-2 pr-4 py-2 flex items-center justify-center rounded-xl">
	<audio
		{src}
		bind:currentTime
		bind:duration
	...
	<p class="text-sm">{currentTimestamp} <span class="text-c-on-bg/50">/</span> {totalTimestamp}</p>
</div>

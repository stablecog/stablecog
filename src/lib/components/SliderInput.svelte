...
>
	...
	>
		<span class="block w-full h-2 rounded-full bg-c-on-bg/20">
			<span
				{...$range}
				class="h-2 rounded-full transition bg-c-on-bg group-active/sliderinput:bg-c-primary"
			/>
		</span>
		{#each [...Array($valueLocal.length).keys()] as _}
			<span
				{...$thumb()}
				class="block ring-0 ring-c-on-bg/25 group-hover/sliderinput:ring-[7px] group-active/sliderinput:ring-c-primary/50
					transition shadow-lg shadow-c-shadow/[var(--o-shadow-strongest)] h-5 w-5 rounded-full bg-c-on-bg
					group-active/sliderinput:bg-c-primary group-active/sliderinput:ring-4 outline-none"
			/>
		{/each}
	</span>
</div>
